...
/* main() sums 16 values from stdin and prints the sum */
main:
    push {lr}    // save lr 
    //Code goes here

    sub sp, sp, #64 @ allocate space for stack pointer (4 * number of values (16))
    mov r4, #0      @ set sum to 0
    mov r5, sp      @ set to sp
    mov r6, #16     @ set number of input values
...
loop_sum:           @ loop to calculate sum
    cmp r6, #0      
    beq done_avg    @ if limit is reached branch to done_sum   
...
done_avg:           
    asr r4, r4, #4  @ divide by 16 to calculate average
    ldr r0, =prtfmt @ format average and print to stdout
    mov r1, r4
    bl printf

    add sp, sp, #64 @ deallocate space used for sp
    mov r0, #0      
    pop {pc}         // put lr in pc    
