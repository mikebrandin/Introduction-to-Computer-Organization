@ Mike Brandin CPSC 2310-002 Lab 10
	.syntax unified
	.arch armv7-a
	.eabi_attribute 27, 3
	.fpu vfpv3-d16
...
put:
	...
	@ CODE HERE
	ldr r1, .L12 		 @ place count pointer in r1
	ldr r1, [r1] 		 @ dereference		
	mov r5, r1			
	cmp r5, #8			 @ branch IF count = 8
	beq if 

	ldr r3, .L12+12		 @ set buffers 
	ldr r2, .L12+8
	ldr r2, [r2]		 @ store buffer	
	strb r0, [r3,r2] 		
	add r1, r1, #1		 @ count += 1
	add r2, r2, #1 		 @ in += 1
	and r2, r2, #7

	ldr r4, .L12+8		 @ update in and count
	str r2, [r4]
	ldr r4, .L12 
	str r1, [r4]
...
if: 					@ if full print error message
	ldr r0, =.LC2 		@ full format
	bl printf
	mov r0, #0
	pop	{r4, r5, pc}    @ restore registers (pop to pc also returns to calling function)
...
get:
	...
	@ CODE HERE
	ldr r3, .L17		@ dereference count held variable
	ldr r3, [r3]
	cmp r3, #0			@ iff count = 0 branch 
	beq if_get
	@else
	sub r3, r3, #1		@ count -= 1 
	ldr r2, .L17+8		@ load buffer and store in r0
	ldr r1, .L17+12
	ldr r2, [r2]	
	ldrb r0, [r1, r2]
	add r2, r2, #1		@ out += 1
	...
	ldr r4, .L17		@ load/store out and count
	str r3, [r4]
	ldr r4, .L17+8
	str r2, [r4]

	pop	{r4, pc}
if_get:					@ print error message
	ldr r0, =.LC0		@ format empty
	bl printf
